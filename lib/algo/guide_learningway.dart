...
void findpre<T extends KnowledgePoint>(MyGraphNode<T> node,Set<MyGraphNode<T>> preset)
{
  ...
  //递归至无父节点
  for(final parent in node.parents as List<MyGraphNode<T>>)
  {
...
void tuopu_DFS<T extends KnowledgePoint>(
    MyGraphNode<T> node,
    Set<MyGraphNode<T>> visited,
    Set<MyGraphNode<T>> subNode,
    MyStack<MyGraphNode<T>> pathStack)
{
  //确保仅在子图内进行排序
  ...
  {
    for (final child in node.children as List<MyGraphNode<T>>)
    {
...
MyQueue<MyGraphNode<T>>? pathPlan<T extends KnowledgePoint>(MyGraph<T> tree, String targetName)
{
  //找到目标知识点
  final MyGraphNode<T>? target=tree.dfsFind(tree.root,targetName);
  if (target==null) 
  {
    print("没找到该学习目标内容");
    return null;
  ...
  final sonNodes=<MyGraphNode<T>>{};   //初始化一个子图来接收所有必要的前置知识点
  findpre(target,sonNodes);  //从目标节点开始反向遍历
  sonNodes.add(target);   //目标节点也加入，保证完整性
  
  //初始化一个集合来跟踪已访问的节点
  final visited=<MyGraphNode<T>>{};
  //初始化一个栈来存储最终的学习路径
  final PathStack = MyStack<MyGraphNode<T>>();
  ...
  // 直接把出栈顺序加入队列
  final lq=MyQueue<MyGraphNode<T>>();
